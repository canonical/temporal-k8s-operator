...
- `temporal-k8s` is deployed and currently blocked, awaiting a database.
...
```
Wait until the application is ready - when it is ready, it will show::
...
At this point, the server may still show schema is not ready. This is expected; the schema will be created after deploying and relating Temporal Admin in the next step.
