# Configure ingress with Nginx Ingress Integrator (deprecated)

[warning]

**This integration is deprecated and will be removed in a future release.**

The `nginx-route` interface used in this guide is on its way out. Migrate to the
standard `ingress` interface instead - see
[Configure ingress with the `ingress` interface](https://charmhub.io/temporal-k8s/docs/h-configure-ingress).
New deployments should not adopt the `nginx-ingress-integrator` path.
