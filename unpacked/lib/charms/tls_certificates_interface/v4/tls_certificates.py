...
class _Certificate(pydantic.BaseModel):
    ...
    def to_provider_certificate(self, relation_id: int) -> "ProviderCertificate":
        ...
        return ProviderCertificate(
            ...
            certificate_signing_request=CertificateSigningRequest.from_string(self.certificate_signing_request),
            ca=Certificate.from_string(self.ca),
            chain=[Certificate.from_string(certificate) for certificate in self.chain] if self.chain else [],
...
class Certificate:
    ...
    def from_string(cls, certificate: str) -> "Certificate":
        ...
        organization_name = certificate_object.subject.get_attributes_for_oid(NameOID.ORGANIZATION_NAME)
        organizational_unit = certificate_object.subject.get_attributes_for_oid(NameOID.ORGANIZATIONAL_UNIT_NAME)
        email_address = certificate_object.subject.get_attributes_for_oid(NameOID.EMAIL_ADDRESS)
        sans_dns: List[str] = []
        sans_ip: List[str] = []
        sans_oid: List[str] = []
        try:
            sans = certificate_object.extensions.get_extension_for_class(x509.SubjectAlternativeName).value
            for san in sans:
        ...
        return cls(
            ...
            state_or_province_name=str(state_or_province_name[0].value) if state_or_province_name else None,
    ...
    def matches_private_key(self, private_key: PrivateKey) -> bool:
        ...
        try:
            ...
            key_object = serialization.load_pem_private_key(private_key.raw.encode(), password=None)
...
class CertificateSigningRequest:
    ...
    def from_string(cls, csr: str) -> "CertificateSigningRequest":
        ...
        state_or_province_name = csr_object.subject.get_attributes_for_oid(NameOID.STATE_OR_PROVINCE_NAME)
        ...
        organizational_unit = csr_object.subject.get_attributes_for_oid(NameOID.ORGANIZATIONAL_UNIT_NAME)
    ...
    def matches_private_key(self, key: PrivateKey) -> bool:
        ...
        try:
            ...
            key_object = serialization.load_pem_private_key(data=key.raw.encode("utf-8"), password=None)
...
def generate_csr(  # noqa: C901
    ...
    if add_unique_id_to_subject_name:
        ...
        subject_name.append(x509.NameAttribute(x509.NameOID.X500_UNIQUE_IDENTIFIER, str(unique_identifier)))
    if organization:
        subject_name.append(x509.NameAttribute(x509.NameOID.ORGANIZATION_NAME, organization))
    if organizational_unit:
        subject_name.append(x509.NameAttribute(x509.NameOID.ORGANIZATIONAL_UNIT_NAME, organizational_unit))
    if email_address:
    ...
    if state_or_province_name:
        subject_name.append(x509.NameAttribute(x509.NameOID.STATE_OR_PROVINCE_NAME, state_or_province_name))
    if locality_name:
...
def generate_ca(
    ...
    """
    private_key_object = serialization.load_pem_private_key(str(private_key).encode(), password=None)
    ...
    subject_identifier_object = x509.SubjectKeyIdentifier.from_public_key(private_key_object.public_key())
...
def _generate_subject_alternative_name_extension(
    ...
    try:
        ...
        sans.extend([x509.DNSName(name) for name in loaded_san_ext.value.get_values_for_type(x509.DNSName)])
        sans.extend([x509.IPAddress(ip) for ip in loaded_san_ext.value.get_values_for_type(x509.IPAddress)])
        sans.extend([x509.RegisteredID(oid) for oid in loaded_san_ext.value.get_values_for_type(x509.RegisteredID)])
        sans.extend([x509.RFC822Name(name) for name in loaded_san_ext.value.get_values_for_type(x509.RFC822Name)])
    except x509.ExtensionNotFound:
        pass
...
class TLSCertificatesRequiresV4(Object):
    ...
    def _remove_requirer_csr_from_relation_data(self, csr: CertificateSigningRequest) -> None:
        ...
        try:
            _RequirerData(certificate_signing_requests=new_relation_data).dump(relation.data[app_or_unit])
        ...
        for csr in requirer_relation_data.certificate_signing_requests:
            requirer_csrs.append(
                RequirerCertificateRequest(
                    ...
                    certificate_signing_request=CertificateSigningRequest.from_string(csr.certificate_signing_request),
    ...
    def _request_certificate(self, csr: CertificateSigningRequest, is_ca: bool) -> None:
        ...
        if not relation:
            logger.debug("No relation: %s", self.relationship_name)
            return
        new_csr = _CertificateSigningRequest(certificate_signing_request=str(csr).strip(), ca=is_ca)
        app_or_unit = self._get_app_or_unit()
        try:
            requirer_relation_data = _RequirerData.load(relation.data[app_or_unit])
        except DataValidationError:
    ...
    def _find_certificate_in_relation_data(self, csr: RequirerCertificateRequest) -> Optional[ProviderCertificate]:
        ...
        for provider_certificate in self.get_provider_certificates():
            if provider_certificate.certificate_signing_request == csr.certificate_signing_request:
                ...
                if not provider_certificate.certificate.matches_private_key(self.private_key):
                    logger.warning("Certificate does not match the private key. Ignoring invalid certificate.")
                    continue
    ...
    def _find_available_certificates(self):
        ...
        for provider_certificate in provider_certificates:
            if provider_certificate.certificate_signing_request in csrs:
                secret_label = self._get_csr_secret_label(provider_certificate.certificate_signing_request)
                if provider_certificate.revoked:
                    ...
                    try:
                        ...
                        if secret.get_content(refresh=True).get("certificate", "") == str(
                            ...
                            logger.debug("Secret %s with correct certificate already exists", secret_label)
                            continue
                        secret.set_content(
                            content={
    ...
    def _cleanup_certificate_requests(self):
        ...
        for requirer_csr in self.get_csrs_from_requirer_relation_data():
            ...
            elif self.private_key and not requirer_csr.certificate_signing_request.matches_private_key(
                self.private_key
            ):
                self._remove_requirer_csr_from_relation_data(requirer_csr.certificate_signing_request)
                logger.info("Removed CSR from relation data because it did not match the private key")  # noqa: E501
...
class TLSCertificatesProvidesV4(Object):
    ...
    def _remove_certificates_for_which_no_csr_exists(self) -> None:
        provider_certificates = self.get_provider_certificates()
        requirer_csrs = [request.certificate_signing_request for request in self.get_certificate_requests()]
        for provider_certificate in provider_certificates:
    ...
    def _get_tls_relations(self, relation_id: Optional[int] = None) -> List[Relation]:
        return (
            [relation for relation in self.model.relations[self.relationship_name] if relation.id == relation_id]
            if relation_id is not None
            else self.model.relations.get(self.relationship_name, [])
        )

    def get_certificate_requests(self, relation_id: Optional[int] = None) -> List[RequirerCertificateRequest]:
    ...
    def _add_provider_certificate(
        ...
        if chain[0] != str(provider_certificate.certificate):
            ...
            logger.warning("The order of the chain from the TLS Certificates Provider is partially incorrect.")
    ...
    def _remove_provider_certificate(
        ...
        for provider_certificate in provider_certificates:
            ...
            if certificate_signing_request and provider_certificate.certificate_signing_request == str(
                certificate_signing_request
            ):
                provider_certificates.remove(provider_certificate)
        self._dump_provider_certificates(relation=relation, certificates=provider_certificates)
    ...
    def get_issued_certificates(self, relation_id: Optional[int] = None) -> List[ProviderCertificate]:
    ...
    def get_provider_certificates(self, relation_id: Optional[int] = None) -> List[ProviderCertificate]:
    ...
    def get_unsolicited_certificates(self, relation_id: Optional[int] = None) -> List[ProviderCertificate]:
    ...
    def _certificate_issued_for_csr(self, csr: CertificateSigningRequest, relation_id: Optional[int]) -> bool:
