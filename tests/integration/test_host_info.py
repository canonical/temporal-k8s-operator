...
class TestTemporalHostInfoRelation:
    async def test_relation(self, ops_test: OpsTest, host_info_requirer: str | Path):
        ...
        await ops_test.model.applications[APP_NAME].set_config(new_cfg)
        ...
        await ops_test.model.wait_for_idle(
            apps=["host-info-requirer"], status="waiting", raise_on_blocked=False, timeout=300
        )
        await ops_test.model.integrate("host-info-requirer:temporal-host-info", f"{APP_NAME}:temporal-host-info")
        await ops_test.model.wait_for_idle(
            apps=["host-info-requirer"], status="active", raise_on_blocked=False, timeout=300
        )
